  ...
  template <typename Iterator> void bulk_load(Iterator ibegin, Iterator iend) {
    ...
    for (size_t i = 0; i < num_parents; ++i) {
      ...
      for (unsigned short s = 0; s < n->slotuse; ++s) {
        ...
        n->childid[s] = leaf;
        n->child_size[s] = leaf->slotuse;
        n->size += leaf->slotuse;
        leaf = leaf->next_leaf;
      }
      n->childid[n->slotuse] = leaf;
      n->child_size[n->slotuse] = leaf->slotuse;
      n->size += leaf->slotuse;
    ...
    for (int level = 2; num_parents != 1; ++level) {
      ...
      for (size_t i = 0; i < num_parents; ++i) {
        ...
        for (unsigned short s = 0; s < n->slotuse; ++s) {
          std::tie(n->childid[s], key_addr, n->child_size[s]) = nextlevel[inner_index];
          n->slotkey[s] = *key_addr;
          n->size += n->child_size[s];
          ++inner_index;
        }
        std::tie(n->childid[n->slotuse], key_addr, n->child_size[n->slotuse]) = nextlevel[inner_index];
        n->size += n->child_size[n->slotuse];
