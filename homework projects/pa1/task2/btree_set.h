...
public:
  ...
  void swap(btree_set &from) { std::swap(tree_, from.tree_); }
...
public:
  ...
  unsigned int count_prefix(const key_type &key) const { return tree_.count_prefix(key); }
...
public:
  ...
  bool operator<(const btree_set &other) const { return (tree_ < other.tree_); }
