...
#define tlx_die_unless(X)                                                      \
  do {                                                                         \
    if (!(X)) {                                                                \
      ::tlx::die_with_message("DIE: Assertion \"" #X "\" failed!", __FILE__,   \
                              __LINE__);                                       \
    }                                                                          \
  } while (false)
...
template <> inline bool die_equal_compare(const char *a, const char *b) {
  // compare string contents
  return std::strcmp(a, b) == 0;
}

template <> inline bool die_equal_compare(float a, float b) {
  // special case for NAN
  return a != a ? b != b : a == b;
}

template <> inline bool die_equal_compare(double a, double b) {
  // special case for NAN
  return a != a ? b != b : a == b;
}
...
template <typename Type> inline Type die_unequal_eps_abs(const Type &t) {
  return t < 0 ? -t : t;
}
